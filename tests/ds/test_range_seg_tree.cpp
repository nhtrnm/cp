...
TEST_CASE(range_sum_seg_tree_build_and_query)
{
    cp::RangeSumSegTree<cp::ll> st({1, 2, 3, 4, 5});
    EXPECT_EQ(st.query(0, 4), 15LL);
    EXPECT_EQ(st.query(1, 3), 9LL);
    EXPECT_EQ(st.query(2, 2), 3LL);
}

TEST_CASE(range_sum_seg_tree_range_update)
{
    cp::RangeSumSegTree<cp::ll> st({1, 2, 3, 4, 5});
    st.update(1, 3, 10);
    // [1, 12, 13, 14, 5]
...
TEST_CASE(range_sum_seg_tree_overlapping_updates)
{
    cp::RangeSumSegTree<cp::ll> st(5);
...
TEST_CASE(range_sum_seg_tree_default_constructed)
{
    cp::RangeSumSegTree<cp::ll> st(5);
...
TEST_CASE(range_sum_seg_tree_assertions)
{
    cp::RangeSumSegTree<cp::ll> st(5);
